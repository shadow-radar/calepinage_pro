...
function calculerFacade(f) {
  ...
  // --- Matériau (famille/sous-type) ---
  var mat      = getMateriau(f.famille, f.sousType) || { tauxChute: 0.05, famille: f.famille };
  var estMetal = mat.famille === 'metallique';
  ...
  // --- Lames ---
  var coeffMat = mat.tauxChute || 0.05;
  ...
  var nbJD      = estMetal ? 0 : Math.floor(larg / 6000);
  var mlJD      = nbJD * (hPose / 1000);
  var sensePose = estMetal
    ? (f.coteVent === 'gauche' ? 'G→D (joint côté terre D)' : 'D→G (joint côté terre G)')
    : 'Bords → Centre (symétrique)';

  return {
    nom: f.nom || ('Façade ' + f.num),
    type: type, larg: larg, largHG: largHG, largHD: largHD,
    hg: hgVal, hd: hdVal, hc: hcVal,
    lu: lu, hu: hu, joint: joint, arH: arH, hPose: hPose, lAir: lAir,
    famille: f.famille, sousType: f.sousType,
    sensePose: sensePose, formatPP: formatPP,
    sBrute: sBrute, sOuv: sOuv, sUtile: sUtile, sAchetee: sAchetee, tauxChute: tauxChute,
    nbTotal: nbTotal, nbEntieres: nbEntieres, nbDecoupees: nbDecoupees,
    nbMontants: nbMontants, nbMontants3m: nbMontants3m, nbEquerres: nbEquerres,
    nbAR: nbAR, nbARBas: nbARBas, nbARHaut: nbARHaut,
    nbVis: nbVis, nbRouleaux: nbRouleaux, nbJD: nbJD, mlJD: mlJD,
    tpsMin: tpsMin, tpsMax: tpsMax,
...
if (typeof window !== 'undefined') {
  window.calculerFacade = calculerFacade;
}
