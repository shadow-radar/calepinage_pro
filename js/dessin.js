...
function dessiner(res) {
  ...
  var W      = Math.max(wrap.clientWidth || window.innerWidth - 40, 300);
  ...
  // Couleurs par famille de matériau (mode pro)
  var famille = (res.famille || 'composite').toLowerCase();
  var colors = {
    composite: {
    ...
    sandwich: {
      entiere: ['#E0E7EF', '#C7D2E0', '#AFC0D6', '#94A9C4'],
      ombre: 'rgba(40,50,70,0.18)',
      reflet: 'rgba(255,255,255,0.2)'
    },
    bois: {
      entiere: ['#C9A66B', '#B8935A', '#A17F49', '#8B6B3A'],
      ombre: 'rgba(60,40,15,0.2)',
      reflet: 'rgba(255,255,255,0.12)'
    }
  };
  var matColors = colors[famille] || colors.composite;
  ...
  if (renderMode === 'pro') {
    ...
    ctx.textAlign = 'center';
    var matInfo  = (typeof getMateriau === 'function') ? getMateriau(res.famille, res.sousType) : null;
    var matLabel = matInfo ? matInfo.label : (res.famille || '');
