...
async function refreshMeteo() {
  ...
  meteoActuelle = await chargerMeteoChantier(projet.chantier);
  afficherMeteoDashboard(meteoActuelle);
  afficherPlanning();
}
...
// --- Rafraîchit uniquement le formulaire d'une façade (garde l'accordéon ouvert) ---
function rafraichirFacadeCard(idx) {
  var body = document.getElementById('fb_' + idx);
  if (!body) return;
  body.innerHTML = renderFacadeForm(facades[idx], idx);
  bindFacadeForm(facades[idx], idx);
  var badgeHost = body.closest('.facade-card').querySelector('.facade-title');
  if (badgeHost) badgeHost.textContent = nomFacade(facades[idx]);
}
...
function renderFacadeForm(f, idx) {
  ...
  return (
    ...
    '<div class="grid2">' +
    '<div class="field"><label>Famille</label><select data-fid="'+f.id+'" data-ff="famille" data-facidx="'+idx+'">' +
    listerFamilles().map(function(fam){
      return '<option value="'+fam.code+'"'+(f.famille===fam.code?' selected':'')+'>'+fam.label+'</option>';
    }).join('')+'</select></div>' +
    '<div class="field"><label>Sous-type</label><select data-fid="'+f.id+'" data-ff="sousType" data-facidx="'+idx+'">' +
    listerSousTypes(f.famille).map(function(st){
      return '<option value="'+st.code+'"'+(f.sousType===st.code?' selected':'')+'>'+st.label+'</option>';
    }).join('')+'</select></div>' +
    ...
    '</div>' +
    (function(){
      var m = getMateriau(f.famille, f.sousType);
      return m ? '<div class="mat-info">💶 ' + m.prixMin + '–' + m.prixMax + ' €/m² (moy. ' + m.prixMoyen + ' €) · ' + m.typePose + '</div>' : '';
    })() +
    '</div>' +
    ...
    '</select></div>' +
    '<div class="field" id="fCV_'+idx+'" style="display:'+(f.famille==='metallique'?'flex':'none')+'"><label>Côté vent</label>' +
...
function bindFacadeForm(f, idx) {
  ...
  body.querySelectorAll('[data-ff]').forEach(function(el) {
    el.addEventListener('change', function() {
      ...
      if (ff === 'famille') {
        // Changement de famille : réappliquer le premier sous-type disponible
        var sousTypes = listerSousTypes(this.value);
        var premierSousType = sousTypes.length ? sousTypes[0].code : '';
        appliquerMateriau(fac, this.value, premierSousType);
        rafraichirFacadeCard(idx);
        return; // rafraichirFacadeCard réaffiche déjà tout, on s'arrête là
      }
      if (ff === 'sousType') {
        appliquerMateriau(fac, fac.famille, this.value);
        rafraichirFacadeCard(idx);
        return;
      }
...
function afficherPlanning() {
  ...
  var etapes = [
    ...
    { key: 'prep', label: '🔧 Préparation', date: dates.prep },
  ...
  etapes.forEach(function(e) {
    if (e.date) {
      ...
      var statut = meteoActuelle ? meteoPourDate(meteoActuelle, e.date) : null;
      var meteoBadge = statut
        ? ' <span class="meteo-chip meteo-' + statut.statut + '">' + statut.label + '</span>'
        : '';
      html += '<div class="planning-item ' + (isPast ? 'past' : '') + ' ' + (isToday ? 'today' : '') + '">';
      html += '<div class="planning-dot"></div>';
      html += '<div class="planning-info">';
      html += '<div class="planning-label">' + e.label + meteoBadge + '</div>';
...
// --- Planning : câblage des inputs de dates ---
function initPlanning() {
  ['planMetreDate', 'planPrepDate', 'planPoseDate', 'planReceptionDate'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.addEventListener('change', afficherPlanning);
  });
}
